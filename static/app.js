...
function setPdfTipo(t) {
  ...
  // Mostrar/ocultar hint de múltiples PDFs
...
async function handlePdfFiles(files) {
  ...
  try {
    const pdfs = await Promise.all(files.map(f => new Promise((res, rej) => {
    ...
    setStatus('Error leyendo los archivos: ' + err.message, 'err');
  }
}
...
async function extractFromPdf(pdfs, filename) {
  ...
  try {
    const resp = await fetch('/analizar-pdf', {
      ...
      body: JSON.stringify({ tipo_doc, pdfs: Array.isArray(pdfs) ? pdfs : [pdfs] })
    });
...
function showPdfResult(data, filename) {
  ...
  const rows = [
    ...
    data.total_comisiones  && ['Total comisiones',  fmtE(data.total_comisiones)],
